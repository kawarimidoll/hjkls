...
impl Backend {
    ...
    fn collect_style_hints(&self, tree: &Tree, source: &str) -> Vec<Diagnostic> {
        ...
        Self::collect_key_notation_hints_recursive(&root, source, &mut diagnostics);
    ...
    fn normalize_key_notation(key: &str) -> Option<String> {
    ...
    fn collect_key_notation_hints_recursive(
        ...
        if node.kind() == "keycode" {
            if let Ok(text) = node.utf8_text(source.as_bytes()) {
                if let Some(normalized) = Self::normalize_key_notation(text) {
                    let start = node.start_position();
                    let end = node.end_position();
        ...
        let mut cursor = node.walk();
        for child in node.children(&mut cursor) {
            Self::collect_key_notation_hints_recursive(&child, source, diagnostics);
        }
    }
...
mod completion_context_tests {
    ...
    fn test_key_notation_normalization() {
        ...
        assert_eq!(Backend::normalize_key_notation("<CR>"), None);
        assert_eq!(Backend::normalize_key_notation("<Esc>"), None);
        assert_eq!(Backend::normalize_key_notation("<Up>"), None);
        assert_eq!(Backend::normalize_key_notation("<F1>"), None);
        assert_eq!(Backend::normalize_key_notation("<C-a>"), None);
        assert_eq!(Backend::normalize_key_notation("<Leader>"), None);

        // Needs normalization - should return Some
        assert_eq!(
            Backend::normalize_key_notation("<cr>"),
            Some("<CR>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<CR>"),
        ...
        assert_eq!(
            Backend::normalize_key_notation("<esc>"),
            Some("<Esc>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<ESC>"),
        ...
        assert_eq!(
            Backend::normalize_key_notation("<up>"),
            Some("<Up>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<UP>"),
        ...
        assert_eq!(
            Backend::normalize_key_notation("<f1>"),
            Some("<F1>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<tab>"),
            Some("<Tab>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<TAB>"),
        ...
        assert_eq!(
            Backend::normalize_key_notation("<space>"),
            Some("<Space>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<bs>"),
            Some("<BS>".to_string())
        );

        // Modifiers
        assert_eq!(
            Backend::normalize_key_notation("<c-a>"),
            Some("<C-a>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<C-A>"),
        ...
        assert_eq!(
            Backend::normalize_key_notation("<s-tab>"),
            Some("<S-Tab>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<c-s-f1>"),
            Some("<C-S-F1>".to_string())
        );

        // Special identifiers
        assert_eq!(
            Backend::normalize_key_notation("<leader>"),
            Some("<Leader>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<plug>"),
            Some("<Plug>".to_string())
        );
        assert_eq!(
            Backend::normalize_key_notation("<sid>"),
            Some("<SID>".to_string())
        );

        // Unknown keys should return None (no suggestion)
        assert_eq!(Backend::normalize_key_notation("<unknown>"), None);
        assert_eq!(Backend::normalize_key_notation("<x>"), None);
    }
}
